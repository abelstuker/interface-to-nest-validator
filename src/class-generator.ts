...
export function generateValidationClasses(filePath: string): string {
  ...
  let output = `import { ${Array.from(imports).join(
    ", "
  ...
  ts.forEachChild(sourceFile, (node) => {
    if (ts.isInterfaceDeclaration(node) && node.name) {
      const className = node.name.text.replace(/^I/, "") + "Dto";
      ...
      node.members.forEach((member) => {
        if (ts.isPropertySignature(member) && member.name) {
          ...
          output += `  ${propertyName}${isOptional ? "?" : ""}: ${getTypeString(
            member.type
          )};\n\n`;
        }
      });
